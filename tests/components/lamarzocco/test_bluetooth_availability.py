...
async def test_bluetooth_dashboard_updates_when_cloud_disconnected(
    hass: HomeAssistant,
    mock_lamarzocco: MagicMock,
    mock_config_entry: MockConfigEntry,
) -> None:
    """Test dashboard updates via Bluetooth when cloud is disconnected."""
    # Set up with Bluetooth client available
    mock_bluetooth_client = MagicMock()
    mock_lamarzocco.bluetooth_client = mock_bluetooth_client
    ...
    # Mock the get_dashboard_from_bluetooth method
    mock_lamarzocco.get_dashboard_from_bluetooth = AsyncMock()
    ...
    # Verify get_dashboard_from_bluetooth was called
    mock_lamarzocco.get_dashboard_from_bluetooth.assert_called()
    # Verify get_dashboard was NOT called (since we're using Bluetooth)
    assert mock_lamarzocco.get_dashboard.call_count == 0
...
async def test_bluetooth_fallback_to_cloud_on_error(
    hass: HomeAssistant,
    mock_lamarzocco: MagicMock,
    mock_config_entry: MockConfigEntry,
) -> None:
    """Test fallback to cloud when Bluetooth fails."""
    # Set up with Bluetooth client available
    mock_bluetooth_client = MagicMock()
    mock_lamarzocco.bluetooth_client = mock_bluetooth_client
    ...
    # Mock the get_dashboard_from_bluetooth method to fail
    mock_lamarzocco.get_dashboard_from_bluetooth = AsyncMock(
        side_effect=BluetoothConnectionFailed("Connection failed")
    )
    ...
    # Verify get_dashboard_from_bluetooth was called and failed
    mock_lamarzocco.get_dashboard_from_bluetooth.assert_called()
    # Verify get_dashboard WAS called as fallback
    assert mock_lamarzocco.get_dashboard.call_count > 0
